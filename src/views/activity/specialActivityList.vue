<template>
    <div>
        ...
        <Search
            ...
            <el-button type="primary" plain size="mini" @click="jumpTo('AddSpecialActivity', 'add', '_')">添加</el-button>
        </Search>
        <div>
            <template>
                <el-table
                    ...
                    <el-table-column
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button type="primary" plain size="mini" @click="jumpTo('AddSpecialActivity','edit',scope.row.id)">编辑</el-button>
    ...
    export default {
        ...
        methods: {
            ...
            async changeStatus({ status, id }) {
                try {
                    await this.$api.activity.activitiesSetStatus({ status, id });
                } catch (e) {
                    // 后端保存出错，强制变回原状态
            ...
            async getData() {
                try {
                    this.loading = true;
                    const { data } = await this.$api.activity.activitiesList({
                        ...this.query,
            ...
            del(id){
                ...
                if(id=='many'){
                    ...
                    this.$api.activity.activitiesDel({ids: this.columnSelect.join(',')}).then(function () {
                        _this.getData();
                    });
                } else {
                    this.$api.activity.activitiesDel({ id }).then(function () {
                        _this.getData();
                    });
                }
            }
