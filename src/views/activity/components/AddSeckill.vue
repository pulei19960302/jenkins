    ...
    <div class="moreRules-content">
        <el-form ref="form" :model="form" :rules="rules" label-width="140px" size="mini" class="form">
            <el-form-item label="活动批次:" prop="title" :rules="[simpleRule]">
                <el-input v-bind:disabled="readtype=='read'" v-model="form.title" style="width: 300px"></el-input>
            </el-form-item>
            <el-form-item label="开始时间:" prop="start_time" :rules="[simpleRule]">
                <el-date-picker
                    v-bind:disabled="readtype=='read'"
                    style="width: 300px;"
            ...
            <el-form-item label="上架/下架:">
                <el-radio-group v-bind:disabled="readtype=='read'"  v-model="form.status">
                    <el-radio :label="10">上架</el-radio>
                    <el-radio :label="0">下架</el-radio>
                </el-radio-group>
            </el-form-item>
            <el-row>
                <el-form-item prop="goods_ids" :rules="[simpleRule]">
                    <!-- <div>
                        <el-cascader
                            v-bind:disabled="readtype=='read'"
                            placeholder="请选择分类"
                        ...
                        <el-button v-show="readtype!='read'" type="primary" @click="handleSearch(form.category_id)" size="mini">搜索</el-button>
                    </div> -->
                    <el-button plain type="primary"  @click.stop="showChooseGoods = true">关联商品</el-button>
                    ...
                    <el-table
                        ...
                        <span v-show="nameEdit != scope.row.id" class="el-icon-edit-content">
                            {{ scope.row.sort }}
                            <i   v-if="readtype!='read'" class="el-icon-edit icon" @click="edit('nameEdit',scope.row.id)"></i>
                        </span>
        ...
        <div class="complete"   v-if="readtype!='read'">
            <el-button type="success" @click="handelSubmit('form')" >提 交</el-button>
        </div>
        <PreviewImg :visible.sync="visible" :imgSrc="previewImg"></PreviewImg>
        <ChooseGoods :choosedGoodsId.sync="form.goods_ids" :showChooseGoods.sync="showChooseGoods"></ChooseGoods>
    </div>
</template>
...
<script>
    ...
    import ChooseGoods from '@/OMS/components/chooseGoods/chooseGoods'
    ...
    export default {
        ...
        data() {
            ...
            const { readtype } = this.$route.params||'';
            return {
                visible: false,
        ...
        methods: {
            ...
            async seckillDetail(id) {
                try {
                    const { data } = await this.$api.activity.seckillDetail({ id });
                    ...
                    this.form.status=data.status;console.log(data.status)
                } catch (e) {
                    throw new Error(e);
                }
            },
                ...
                this.$refs[formName].validate(async (valid) => {
                    if (valid) {
                        try {
                            ...
                            await this.$api.activity.seckillAdd(params);
