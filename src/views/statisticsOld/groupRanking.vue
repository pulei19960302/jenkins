<template>
    <div class="content">
        <div>
            <el-button :type="topType==2?'primary':''" @click="topType=2">群统计</el-button>
            <el-button :type="topType==1?'primary':''" @click="topType=1">群排行</el-button>
        </div>
        ...
        <section v-show="topType==1">
            <div style="margin-top: 10px">
                ...
                <div class="top-slot">
                    团队总数 <span>{{ teamcount }}</span> 个,  下单总人数 <span>{{ total_order_member }}</span> 单, 消费总额 <span>{{ total_fee }}</span> 元
                </div>
                <template>
                    <el-table
                        element-loading-spinner="el-icon-loading"
    ...
    export default {
        ...
        data() {
            return {
                ...
                total_money:0,
                options: [
                    {value: 'person_num', label: '群总人数'},
                    {value: 'total_fee', label: '群下单总额'},
                    {value: 'order_member', label: '群下单人数'},
                ],
        ...
        methods: {
            ...
            async getData(detail) {
                try {
                    this.loading = true;
                    const { data } = await this.$api.statistics.group({
                        ...this.query,
                        ...detail,
            ...
            async teamrank(detail) {
                try {
                    this.loading = true;
                    const { data } = await this.$api.statistics.grouprank({
                        ...this.query,
                        ...detail,
                        pageSize: this.pageSize,
                        page: this.current,
