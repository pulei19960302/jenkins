<template>
    <div>
        <Search
            ...
            <el-button type="primary" plain size="mini" @click="dialogVisible=true;form={status:10}" v-permission="[$api.order.reason.edit]">添加问题</el-button>
        </Search>
        <!--表单-->
        <template>
            <el-table
        ...
        <el-dialog
            ...
            <el-form
                ...
                <el-form-item label="问题答案："  prop="content">
                    <el-input
                        type="textarea"
                        :rows="4"
    ...
    export default {
        ...
        methods: {
            async switchChange(status, id) {
                await this.$api.service.setstatus({ status, id });
            },
            resetForm() {
                this.$refs.form.resetFields();
            },
            handelEdit({ content, problem, sort, status, id }) {
                this.id = id;
                this.isEdit = true;
                this.dialogVisible = true;
                this.form = { content,sort, problem, status };
                this.oldRowData = { content,sort, problem, status };
            },
            AddReason(isEdit) {
                this.$refs.form.validate(async (valid) => {
                    if (valid) {
                        try {
                            ...
                            await this.$api.service.reasonEdit(params);
                            this.isEdit = false;
            ...
            async getData(){console.log(this.id)
                //获取列表数据
                try {
                    this.loading = true;
                    const { data } = await this.$api.service.reasonlist({
                        ...this.query,
            ...
            handleDel(id) {
                ...
                this.$confirm('确认删除选中的问题？', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(async () => {
                    await this.$api.service.delreasons({ ids });
                    this.getData();
                })
            },
        }
